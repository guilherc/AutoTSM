...
std::pair<int, int> ORIGINAL_RES = { 1920, 1200 };
...
void log_to_file(const std::string& message) {
    try {
        ...
        std::ofstream logfile("automation_log.txt", std::ios_base::app);
        if (logfile.is_open()) {
            ...
            std::cerr << "Failed to open log file! Message: " << message << std::endl;
        }
    }
    catch (...) {
...
std::pair<int, int> adjust_coordinates(int x, int y) {
    ...
    float ratio_x = static_cast<float>(current_res.first) / ORIGINAL_RES.first;
    float ratio_y = static_cast<float>(current_res.second) / ORIGINAL_RES.second;

    // Use the smaller ratio to maintain proportion (avoids distortion)
    float ratio = std::min(ratio_x, ratio_y);

    // Calculate margins to center
    int offset_x = static_cast<int>((current_res.first - (ORIGINAL_RES.first * ratio)) / 2);
    int offset_y = static_cast<int>((current_res.second - (ORIGINAL_RES.second * ratio)) / 2);

    // Apply the adjustment
    int x_adjusted = static_cast<int>(x * ratio) + offset_x;
    int y_adjusted = static_cast<int>(y * ratio) + offset_y;
...
void pause() {
    ...
    std::cout.flush(); // Ensure the message is displayed immediately
...
void check_configure_resolution() {
    ...
    std::cout << " Current reference resolution: " << ORIGINAL_RES.first << "x" << ORIGINAL_RES.second << "\n";
    // Display the saved resolution
    ...
    std::cout << "3. Keep default resolution (" << ORIGINAL_RES.first << "x" << ORIGINAL_RES.second << ")\n";
    std::cout << "0. Back\n\n";
    ...
    switch (choice) {
    case 1: {
        const_cast<std::pair<int, int>&>(ORIGINAL_RES) = current_res;
    ...
    case 2: {
        int width, height;
        std::cout << "\nEnter the width: ";
        while (!(std::cin >> width) || width < 800 || width > 7680) { // Example: minimum 800, maximum 8K
        ...
        while (!(std::cin >> height) || height < 600 || height > 4320) { // Example: minimum 600, maximum 8K
            std::cout << "Invalid height! Enter a number between 600 and 4320: ";
            std::cin.clear();
            std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        }

        const_cast<std::pair<int, int>&>(ORIGINAL_RES) = { width, height };
        std::cout << "\nResolution set to: " << width << "x" << height << "\n";
        save_resolution_json();
        break;
    }
    case 3: {
        std::cout << "\nKeeping default resolution: " << ORIGINAL_RES.first << "x" << ORIGINAL_RES.second << "\n";
        save_resolution_json();
        break;
    }
    case 0:
...
std::string get_exe_path() {
    ...
    if (!PathRemoveFileSpecA(path)) {
        log_error("Error removing file name from path");
        return "";
    }
...
void load_coordinates() {
    ...
    try {
        ...
        // Load the saved resolution, if it exists
        if (data.contains("saved_resolution")) {
            ...
            // If no saved resolution, use the default (1920x1200)
            resolution_loaded = false; 
            saved_resolution = { 0, 0 }; 
            show_progress("No saved resolution found. Using default: " +
                std::to_string(ORIGINAL_RES.first) + "x" +
                std::to_string(ORIGINAL_RES.second));
        }
...
struct Account {
    ...
    std::string password;
    bool heart_donor;
    int friend_count; // 0 if not a donor, 1 to 10 if a donor
};
...
void save_schedules_json() {
    ...
    for (const auto& sch : schedules) {
        j["schedules"].push_back({
            ...
            {"last_execution", sch.last_execution}
            });
    }
...
bool ensure_window_active(HWND hwnd) {
    ...
    if (!is_window_active(hwnd)) {
        ...
        // Verify if the window was successfully activated
        if (!is_window_active(hwnd)) {
...
// Human-like mouse movement between two points
void precise_click(int dest_x, int dest_y, int steps = 50, int duration_ms = 500) {
    ...
    // Bézier curve for smoother movement
    for (int i = 0; i <= steps; i++) {
        float t = static_cast<float>(i) / static_cast<float>(steps);

        // Smooth with Bézier curve (ease-in-out)
        t = t < 0.5f ? 2.0f * t * t : 1.0f - powf(-2.0f * t + 2.0f, 2.0f) / 2.0f;

        // Add small random variations to the path
...
// Human-like click with small variations
void human_click() {
    // Small movement before the click (like a human adjusting)
    POINT pos;
    GetCursorPos(&pos);
...
void collect_mission_candles() {
    std::this_thread::sleep_for(std::chrono::milliseconds(random_range(400, 600)));
    // Ensure the window is active before pressing F
    HWND hwnd = find_window_by_title(L"Sky");
    if (ensure_window_active(hwnd)) {
        press_key('F', random_range(30, 80));
        std::this_thread::sleep_for(std::chrono::milliseconds(random_range(100, 300)));
...
void collect_candles() {
    std::this_thread::sleep_for(std::chrono::milliseconds(random_range(400, 600)));
    precise_click(coordinates["random_game_click"].x, coordinates["random_game_click"].y);
    human_click();
    std::this_thread::sleep_for(std::chrono::milliseconds(random_range(400, 600)));

    // Ensure the window is active before pressing R
    HWND hwnd = find_window_by_title(L"Sky");
    if (ensure_window_active(hwnd)) {
    ...
    for (int i = 0; i < total_time; i += interval) {
        {
            ...
            if (stop_global_flag) {
                log_error("Candle Run - Interrupted by user");
                return;
            }
        }
        ...
        if (!is_process_running(L"Sky.exe")) {
            log_error("Sky.exe process closed - Marking for restart");
            restart_game_in_progress = true;
            return; // Return immediately to restart the flow
        }

        try {
            if (!is_sky_window_active()) {
                attempts++;
                log_error("Window inactive - Attempt " + std::to_string(attempts) + "/" + std::to_string(max_attempts));
                ...
                if (attempts >= max_attempts) {
                    log_error("Maximum attempts reached - Checking if game was closed");
                    if (!is_process_running(L"Sky.exe")) {
                        log_error("Game closed - Marking for restart");
                        restart_game_in_progress = true;
                    }
                    else {
                        
                        log_error("Window inactive but process still exists - Forcing restart");
                        close_game(); // Force close the game
                        restart_game_in_progress = true;
                    }
                    return; // Return immediately to restart the flow
                }
...
void precise_click(int x, int y, const std::string& point_name, int delay_min = 400, int delay_max = 600) {
    ...
    log_load("Click on '" + point_name + "' - Original: (" +
        std::to_string(x) + "," + std::to_string(y) +
        ") Adjusted: (" + std::to_string(adjusted.first) + "," +
        std::to_string(adjusted.second) + ")");

    human_mouse_move(adjusted.first, adjusted.second); 
    human_click(); 
...
void hide_mod() {
    HWND hwnd = find_window_by_title(L"Sky");
    if (ensure_window_active(hwnd)) {
		// Press the Home key - testing with 0x01 as a placeholder for Home key
        press_key(0x01, 50);
        std::this_thread::sleep_for(std::chrono::milliseconds(random_range(100, 300)));
    }
    else {
...
std::string wstring_to_string(const std::wstring& wstr) {
    ...
    // Determine the size needed for the converted string
...
void start_game(const std::string& name, const std::string& username, const std::string& password) {
    ...
    try {
        ...
        std::string login_command = "start \"\" \"" + STEAM_PATH + "\" -login " + username + " " + password;
    ...
    while (elapsed_time < max_wait_seconds * 1000) {
        ...
        if (hwnd) {
            ...
            if (title == L"Sky") {
                ...
                log_load("Window not yet ready: " + wstring_to_string(title));
            }
        }
        else {
    ...
    if (!hwnd || elapsed_time >= max_wait_seconds * 1000) {
        log_error("Sky window not found or not ready in time");
    ...
    try {
        show_progress("Started Game (Play)");
        if (ensure_window_active(hwnd)) {
            for (int i = 0; i < 3; i++) {
                press_key(' ', 30);
        ...
        // Find the account to check heart_donor and friend_count
        auto it = std::find_if(accounts.begin(), accounts.end(),
            [&name](const Account& c) { return c.name == name; });
        if (it == accounts.end()) {
            log_error("Account " + name + " not found in the account list");
        ...
        if (name == "Account 1") {
            show_progress("Completing Daily Missions");
            collect_daily_missions();
            press_key('F', random_range(30, 80));
            std::this_thread::sleep_for(std::chrono::milliseconds(random_range(100, 300)));
            press_key('F', random_range(30, 80));
            std::this_thread::sleep_for(std::chrono::milliseconds(random_range(100, 300)));
            complete_daily_missions();
            show_progress("Collecting Daily Mission Candles");
            collect_mission_candles();
...
void steam_game_workflow(const std::string& name, const std::string& username, const std::string& password, bool individual) {
    show_progress("Starting automation for " + name + " (" + username + ")");
        ...
        try {
            {
                ...
                if (stop_global_flag || (individual && stop_flags[name])) {
                    log_error("Automation interrupted for " + name);
                    return;
                }
            }
            ...
            start_game(name, username, password);

            // If marked for restart, return to the start of the flow
            if (restart_game_in_progress) {
                show_progress("Restarting flow for " + name + " after game closure");
                continue; // Restart the while loop without incrementing attempts
            }

            // If it reaches here, the flow was completed successfully
            break;
        }
        catch (const std::exception& e) {
            attempts++;
    ...
    show_progress("Automation completed for " + username);
...
void start_automation(const std::string& name, const std::string& username, const std::string& password, bool individual = true) {
    std::lock_guard<std::mutex> lock(stop_flags_mutex);
    ...
    std::thread([name, username, password, individual]() {
        steam_game_workflow(name, username, password, individual);
        }).detach();
}
    ...
    std::thread([]() {
        try {
            for (const auto& account : accounts) {
                ...
                // Create a thread for the current account
                ...
                // Wait until the current account finishes or is interrupted
                while (!account_finished && !stop_global_flag) {
                    std::this_thread::sleep_for(std::chrono::seconds(1));

                    // Check if the game was closed unexpectedly
                    if (!is_process_running(L"Sky.exe") && !account_finished) {
                        log_error("Game closed unexpectedly - Restarting flow for " + account.name);
                        break;
                    }
                }
...
void list_accounts() {
    ...
    for (size_t i = 0; i < accounts.size(); i++) {
        ...
        // Account name (max 15 characters, left-aligned)
        std::cout << std::left << std::setw(15) << accounts[i].name;

        // Username (max 15 characters, left-aligned)
        std::cout << " | Username: " << std::setw(15) << accounts[i].username << " |\n";
    }
...
void add_account() {
    ...
    if (new_account.heart_donor) {
        int friends;
        std::cout << "Number of friends (1 to 10): ";
        while (!(std::cin >> friends) || friends < 1 || friends > 10) {
...
void save_resolution_json() {
    ...
    // Update the saved resolution
    data["saved_resolution"] = {
        {"width", ORIGINAL_RES.first},
        {"height", ORIGINAL_RES.second}
    };
...
void add_schedule() {
    ...
    std::getline(std::cin, new_schedule.description);
    if (new_schedule.description.empty()) {
    ...
    std::cout << "Hour (0-23): ";
    while (!(std::cin >> new_schedule.hour) || new_schedule.hour < 0 || new_schedule.hour > 23) {
    ...
    while (!(std::cin >> new_schedule.minute) || new_schedule.minute < 0 || new_schedule.minute > 59) {
    ...
    new_schedule.recurring = (option == 'D');

    new_schedule.active = true;
    new_schedule.last_execution = 0;
...
void remove_schedule() {
    ...
    for (size_t i = 0; i < schedules.size(); i++) {
        const auto& sch = schedules[i];
        std::cout << " " << (i + 1) << ". " << std::setw(2) << std::setfill('0') << sch.hour << ":"
            << std::setw(2) << std::setfill('0') << sch.minute << " - " << sch.description
            << " [" << (sch.active ? "Active" : "Inactive") << "]\n";
    }

    std::cout << "\n0. Back\n";
    std::cout << "Enter the schedule number to remove: ";
...
void edit_schedule() {
    ...
    std::cout << "\n0. Back\n";
    std::cout << "Enter the schedule number to edit: ";
    ...
    if (choice != 0) {
        auto& sch = schedules[choice - 1];
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

        std::cout << "New description [" << sch.description << "]: ";
        std::string new_desc;
        std::getline(std::cin, new_desc);
        if (!new_desc.empty()) sch.description = new_desc;

        std::cout << "New hour [" << sch.hour << "]: ";
        std::string hour_str;
        ...
        if (!hour_str.empty()) {
            ...
            try {
                ...
                if (new_hour >= 0 && new_hour <= 23) sch.hour = new_hour;
        ...
        std::cout << "New minute [" << sch.minute << "]: ";
        ...
        if (!minute_str.empty()) {
            ...
            try {
                ...
                if (new_minute >= 0 && new_minute <= 59) sch.minute = new_minute;
        ...
        std::cout << "Recurring? (D)aily or (O)ne-time [" << (sch.recurring ? "D" : "O") << "]: ";
        char option;
        while (true) {
            std::cin >> option;
            option = toupper(option);
        ...
        if (option == 'D' || option == 'O') sch.recurring = (option == 'D');
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

        std::cout << "Active? (Y)es or (N)o [" << (sch.active ? "Y" : "N") << "]: ";
        while (true) {
            std::cin >> option;
            option = toupper(option);
        ...
        if (option == 'Y' || option == 'N') sch.active = (option == 'Y');
        std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    ...
    while (schedule_running) {
...

        {
            ...
            for (auto& sch : schedules) {
                if (sch.active && sch.hour == now_tm.tm_hour && sch.minute == now_tm.tm_min) {
                    // Check if it was already executed today (to avoid multiple executions)
                    std::tm last_tm = localtime_xp(sch.last_execution);
                    bool same_day = (last_tm.tm_year == now_tm.tm_year &&
                        last_tm.tm_mon == now_tm.tm_mon &&
                        last_tm.tm_mday == now_tm.tm_mday);

                    if (!sch.recurring && sch.last_execution != 0) {
                        // One-time schedule already executed
                        sch.active = false;
                        continue;
                    }

                    if (sch.recurring || !same_day) {
                        show_progress("Executing schedule: " + sch.description);
                        sch.last_execution = now_time;
                        ...
                        // If one-time, deactivate after execution
                        if (!sch.recurring) {
                            sch.active = false;
                        }
    ...
    do {
        ...
        case 1:
            if (!general_cycle_running) {
                ...
                clear_screen();
                header();
                std::cout << "General cycle is already running!\n";
                pause();
            }
            break;
        case 2: {
            ...
            while (!(std::cin >> account_number) || account_number < 0 || account_number > static_cast<int>(accounts.size())) {
...
int main(int argc, char* argv[]) {
    ...
    if (accounts.empty()) {
        log_error("No accounts registered. Add accounts via the menu.");
    }
